...
class Queldorei_Shopperslideshow_Adminhtml_ShopperslideshowController extends Mage_Adminhtml_Controller_Action
{
	...
	public function indexAction() {
		$this->_initAction()
			->_addContent($this->getLayout()->createBlock('shopperslideshow/adminhtml_shopperslideshow'))
			->renderLayout();
	}

	public function editAction() {
		$id     = $this->getRequest()->getParam('id');
		$model  = Mage::getModel('shopperslideshow/shopperslideshow')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register('shopperslideshow_data', $model);

			$this->getLayout()->getBlock('head')->setCanLoadExtJs(true);

			$this->_addContent($this->getLayout()->createBlock('shopperslideshow/adminhtml_shopperslideshow_edit'))
				->_addLeft($this->getLayout()->createBlock('shopperslideshow/adminhtml_shopperslideshow_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			if(isset($_FILES['image']['name']) && $_FILES['image']['name'] != null) {
		        ...
		        $data['image'] = 'queldorei/shopper/slideshow/'.$result['file'];
			}
            ...
            if(isset($_FILES['small_image']['name']) && $_FILES['small_image']['name'] != null) {
		        ...
		        $data['small_image'] = 'queldorei/shopper/slideshow/'.$result['file'];
			}
    ...
    public function massDeleteAction() {
        $shopperslideshowIds = $this->getRequest()->getParam('shopperslideshow');
        if(!is_array($shopperslideshowIds)) {
            ...
            try {
                foreach ($shopperslideshowIds as $shopperslideshowId) {
                    $shopperslideshow = Mage::getModel('shopperslideshow/shopperslideshow')->load($shopperslideshowId);
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($shopperslideshowIds)) {
            ...
            try {
                foreach ($shopperslideshowIds as $shopperslideshowId) {
                    $shopperslideshow = Mage::getSingleton('shopperslideshow/shopperslideshow')
